...
- Anthropic Claude (Opus 4.7) + OpenAI GPT-5.2 dual-review LLM pipeline
- Vitest + Playwright
...
- Required env vars: `DATABASE_URL`, `JWT_SECRET` (≥32 chars), `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `ANTHROPIC_API_KEY`. Add `OPENAI_API_KEY` to enable the OpenAI advocate-review half of the dual-model pipeline (`OPENAI_MODEL` defaults to `gpt-5.2`). Production additionally requires `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`.
